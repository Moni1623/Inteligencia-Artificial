# -*- coding: utf-8 -*-
"""26_UNAM_AI_Actividad_Laboratorio_4_7_precios_casas.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1-OJyZmAtaacXNt798GUyQvg480pSBAqx
...
"""#Parte 3: Métricas de Clasificación aplicadas al modelo de precios

Para evaluar el rendimiento del modelo de regresión desde una perspectiva de clasificación, convertimos el problema en uno binario: definimos un **umbral** basado en la mediana del precio de las casas para etiquetar cada casa como **cara** (1) o **barata** (0). Así podemos calcular la **Matriz de Confusión**, la **Exactitud (Accuracy)** y la **Precisión (Precision)**, de forma análoga a la práctica de métricas (Lab 4.1).

Estas métricas nos permiten ver qué tan bien clasifica el modelo, no sólo qué tan lejos está en valor numérico.
"""
...
# Convertimos a clasificación binaria usando la mediana como umbral
# 1 = casa cara (precio >= mediana), 0 = casa barata (precio < mediana)
umbral = np.median(y)
...
print("--- MÉTRICAS DE CLASIFICACIÓN (umbral = mediana del precio) ---")
print(f"Umbral usado (mediana): {umbral:.2f} miles de dólares")
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
            xticklabels=['Barata (0)', 'Cara (1)'],
            yticklabels=['Barata (0)', 'Cara (1)'])
plt.title('Matriz de Confusión\n(Clasificación de precios de casas)')
...
Al transformar el problema de regresión en uno de clasificación binaria usando la mediana del precio como umbral, pudimos aplicar métricas como la Matriz de Confusión, la Exactitud (Accuracy) y la Precisión (Precision) al modelo de precios de casas. La Matriz de Confusión nos permite identificar visualmente cuántas casas fueron correctamente clasificadas como "caras" o "baratas", así como los errores cometidos (falsos positivos y falsos negativos). El Accuracy nos indica qué porcentaje total de predicciones fueron correctas, mientras que la Precisión nos dice qué tan confiables son las predicciones positivas (casas clasificadas como caras que realmente lo son). Estas métricas complementan a las de regresión (MSE, RMSE, R²) y nos dan una visión más completa del rendimiento del modelo.
