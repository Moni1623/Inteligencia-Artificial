...
Original file is located at
    https://colab.research.google.com/drive/1ID-LkPudKcaz2QBXIQ8ySrUtvNhWIKMO
...
def evaluar_modelo(nombre, modelo, X_train, y_train, X_test, y_test):
    ...
    print("Accuracy validación:", acc_test)
    print("Precision entrenamiento:", prec_train)
    print("Precision validación:", prec_test)
    print("\nMatriz de confusión (validación):")
    print(cm)

# Evaluar modelos
evaluar_modelo("Decision Tree", dt_model, X_train, y_train, X_test, y_test)
evaluar_modelo("Random Forest", rf_model, X_train, y_train, X_test, y_test)
evaluar_modelo("XGBoost", xgb_model, X_train, y_train, X_test, y_test)

"""## Análisis de Métricas

La matriz de confusión muestra:
- VP: malignos bien clasificados  
- VN: benignos bien clasificados  
- FP: benignos clasificados como malignos  
- FN: malignos clasificados como benignos  

Los **falsos negativos** son los más peligrosos, ya que no detectan cáncer.

## Métricas

- **Accuracy:** porcentaje total de aciertos  
- **Precision:** qué tan confiable es cuando predice “maligno”  
...
Modelos como **Random Forest** y **XGBoost** tienen mejor desempeño.  
El árbol de decisión es más simple pero menos preciso.

#Conclusión

Desde mi perspectiva como estudiante de Ciencias de la Computación, este laboratorio me permitió comprender de manera más clara cómo los modelos de aprendizaje automático pueden aplicarse a problemas reales, especialmente en un área tan relevante como la salud. Más allá de implementar los algoritmos, considero que lo más valioso fue entender el impacto que tienen decisiones aparentemente simples, como el manejo de datos faltantes o la elección de parámetros, en el desempeño final de los modelos.\\

También me resultó interesante comparar un modelo sencillo, como el árbol de decisión, con métodos más avanzados como Random Forest y XGBoost. Esto me ayudó a ver que no siempre el modelo más complejo es el más adecuado en todos los contextos, ya que existe un balance importante entre interpretabilidad y precisión, especialmente cuando se trata de aplicaciones médicas donde es necesario justificar las decisiones.\\

Finalmente, este ejercicio reforzó mi interés en el área de inteligencia artificial, ya que muestra cómo, a partir de datos bien procesados, es posible construir herramientas que apoyen la toma de decisiones en contextos críticos. Considero que este tipo de prácticas no solo fortalecen las habilidades técnicas, sino también la capacidad de analizar problemas de forma integral.\\
